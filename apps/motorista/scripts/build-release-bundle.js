/**
 * Gera AAB (Android App Bundle) release do app Motorista no PC.
 * Faz bump da versão (patch) no package.json, passa versão ao Gradle e gera
 * android/app/build/outputs/bundle/release/take-me-motorista-{versão}.aab
 */
...
// Força o Metro/Expo a usar apps/motorista como raiz no monorepo
const buildEnv = {
  ...process.env,
...
if (result.status === 0) {
  ...
  const aabName = `take-me-motorista-${newVersion}.aab`;
