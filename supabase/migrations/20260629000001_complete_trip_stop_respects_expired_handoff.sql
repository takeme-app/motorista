-- Quando o handoff do preparador expira (1h antes da partida sem nenhum preparador
-- reclamar a encomenda), o motorista assume a coleta direta com o cliente — mesmo
-- que `shipments.base_id` continue setado.
--
-- O cliente valida via `complete_shipment_client_pickup` (já corrigida na migration
-- 20260629000000) e seta `picked_up_at`. Depois disso, o motorista clica em
-- "Cliente confirmou — seguir", que chama esta RPC.
--
-- A versão anterior sempre tomava o branch "com base" quando `sh_base_id IS NOT NULL`,
-- exigindo `base_to_driver_confirmed_at` e o PIN da base — nada disso existe no fluxo
-- direto. Resultado: erro 'pickup_not_confirmed' mesmo com o cliente já tendo validado.
--
-- Esta migration trata "base_id setado + handoff expirado" como fluxo sem base.
...
BEGIN
  ...
  IF tnorm IN (
    ...
    IF tnorm IN ('package_pickup', 'shipment_pickup') AND NOT sh_effective_has_base THEN
      -- Sem base (ou handoff do preparador expirado): cliente confirma pickup_code no app dele
      -- e seta `picked_up_at`. Motorista não digita PIN, mas só pode concluir após o cliente confirmar.
    ...
    ELSIF tnorm IN ('package_pickup', 'shipment_pickup') AND sh_effective_has_base THEN
      -- Com base: motorista digita o `base_to_driver_code` (PIN C) que a base
      -- passou. Antes não comparávamos — permitia "concluir" a encomenda errada
      -- quando havia mais de uma na base.
      ...
      END IF;
      exp_digits := regexp_replace(coalesce(sh_base_to_driver, ''), '\D', '', 'g');
