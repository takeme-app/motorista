-- Adiciona aviso de "Excursão cancelada" ao phase-change (antes só via genérico).
-- Demais ramos preservados; bloco guardado por OLD.status IS DISTINCT FROM NEW.status.
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    ELSIF NEW.status = 'cancelled' THEN
      IF public.should_notify_user(v_user, 'excursions') THEN
        INSERT INTO public.notifications (user_id, title, message, category, target_app_slug, data)
        VALUES (v_user, 'Excursão cancelada', 'Sua excursão foi cancelada. Caso tenha pago, o estorno é automático no cartão (5 a 10 dias). Em caso de dúvida, fale com o suporte.', 'excursions', 'cliente', v_data);
      END IF;
    END IF;
  END IF;
