...
BEGIN
  ...
  SELECT s.preparer_id, s.base_id, s.picked_up_by_preparer_at,
         s.preparer_to_base_code, s.base_to_driver_code, s.delivered_to_base_at
    INTO v_preparer_id, v_base_id, v_picked_up_preparer,
         v_preparer_to_base_code, v_base_to_driver_code, v_already
  ...
  IF length(v_pin_b_digits) <> 4 AND length(v_pin_c_digits) <> 4 THEN
    RETURN jsonb_build_object('ok', false, 'error', 'missing_code');
  END IF;

  IF v_digits_in IS DISTINCT FROM v_pin_b_digits
     AND v_digits_in IS DISTINCT FROM v_pin_c_digits THEN
...
COMMENT ON FUNCTION public.complete_shipment_preparer_to_base (uuid, text) IS
  'Confirma depósito na base: aceita o PIN B do handoff preparador-base ou o PIN operacional da base quando esse for o código fornecido ao preparador.';
