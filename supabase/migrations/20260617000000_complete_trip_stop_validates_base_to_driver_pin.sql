-- Para shipment_pickup-com-base, o backend só checava
-- `base_to_driver_confirmed_at IS NOT NULL` — não comparava o PIN digitado
-- pelo motorista. Quando havia 2+ encomendas na mesma base, isso permitia
-- "concluir" a errada (o front também não validava — fix em ActiveTripScreen).
-- Esta migration adiciona validação dura: motorista precisa digitar o
-- `base_to_driver_code` exato da encomenda que está retirando.
...
BEGIN
  ...
  IF tnorm IN (
    ...
    IF tnorm IN ('package_pickup', 'shipment_pickup') AND sh_base_id IS NULL THEN
      -- Sem base: cliente confirma pickup_code no app dele e seta `picked_up_at`.
      -- Motorista não digita PIN, mas só pode concluir após o cliente confirmar.
