-- Regra de produto (2026-05-28): retirada na base passa a ser "motorista mostra
-- código, base recebe, motorista confirma" — sem PIN digitado e sem exigir que
-- a base tenha tocado "confirmar" no app dela. O motorista é responsável pela
-- conferência.
--
-- Esta migration reescreve `complete_trip_stop` removendo, para
-- `package_pickup` com base efetiva, a exigência de `base_to_driver_confirmed_at`
-- e do PIN de 4 dígitos. Os demais ramos (entrega, sem base, passageiro,
-- dependente) permanecem com a validação original.
...
BEGIN
  ...
  IF tnorm IN (
    ...
    ELSIF tnorm IN ('package_pickup', 'shipment_pickup') AND sh_effective_has_base THEN
      -- Retirada na base: sem PIN e sem exigir base_to_driver_confirmed_at.
      -- Motorista mostra o código à base e confirma manualmente o avanço.
      NULL;
    ELSE
