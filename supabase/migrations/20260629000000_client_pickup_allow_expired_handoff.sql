-- Quando a janela de 1h do preparador expira sem ele reclamar a encomenda,
-- o motorista assume a coleta direta (ver 20260608000000_shipment_preparer_handoff_timeout.sql).
-- Nesse cenário, cliente e motorista usam o fluxo "sem base" (PIN pickup_code),
-- mas o RPC original bloqueava com 'has_base' sempre que base_id estava setado,
-- ignorando preparer_handoff_expired_at. Resultado: cliente recebia o alerta
-- "Fluxo com base" mesmo quando deveria validar o código direto do motorista.
...
BEGIN
  ...
  SELECT s.user_id, s.base_id, s.preparer_handoff_expired_at, s.driver_id,
         s.status, s.pickup_code, s.picked_up_at
    INTO v_user_id, v_base_id, v_handoff_expired, v_driver_id,
         v_status, v_expected, v_picked_up_at
  ...
  IF v_base_id IS NOT NULL AND v_handoff_expired IS NULL THEN
...
COMMENT ON FUNCTION public.complete_shipment_client_pickup (uuid, text) IS
  'Encomenda sem base (ou com handoff do preparador expirado): cliente valida o PIN de coleta informado pelo motorista e marca picked_up_at/status in_progress.';
